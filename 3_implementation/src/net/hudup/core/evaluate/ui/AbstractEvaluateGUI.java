...
public abstract class AbstractEvaluateGUI extends JPanel implements EvaluatorListener, EvaluateListener, EvaluateProgressListener, SetupAlgListener, PluginChangedListener, CounterElapsedTimeListener, ClassProcessor, Serializable {
	 ...
	 * @return true if connection to server is successful;
	 */
	protected synchronized boolean taskQueueFeed() {
		...
		for (EventObject evt : evtList) {
			try {
				...
				else if (evt instanceof CounterElapsedTimeEvent) {
					receivedElapsedTime((CounterElapsedTimeEvent)evt);
				}
			}
			catch (Throwable e) {LogUtil.trace(e);}
		}
	...
	public Metrics getResult(boolean alsoUpdate) {
		...
		try {
			...
			if (tempResult != null) {
				recoveredResult = result = tempResult;
				return tempResult;
			}
	...
	public void dispose() {
		...
		if (connectInfo.bindUri != null) { //Remote evaluator.
			unsetupListeners(evaluator);
			
			if (EvaluatorAbstract.isWrapper(evaluator)) {
				try {
					evaluator.close();
				}
				catch (Exception e) {LogUtil.trace(e);}
